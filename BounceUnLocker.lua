...
function main()
	...
	sampAddChatMessage("{7044EE}[Bounce Script (DevOps)]: {2DA440}Ñêğèïò óñïåøíî çàãğóæåí! {EDEFE0}Îæèäàåì ïîäêëş÷åíèÿ íà ñåğâåğ...", -1)
	repeat wait(0) until sampGetGamestate() == 3
	sampAddChatMessage("{7044EE}[Bounce Script (DevOps)]: {2DA440}Ïîäêëş÷åíû. {C1973F}Àâòîğû: electron.js. {EDEFE0}Àêòèâàöèÿ àâòîìàòè÷åñêàÿ.", -1)
	...
	if isSampLoaded() then
		if isSampfuncsLoaded() then
			...
			print("SAMPFUNC òğåáóåòñÿ äëÿ ğàáîòû â SAMP")
			samp = 1
		end
	end
	...
	while true do
		...
		if flag then
			...
			local zone = GetPlayerInZones(getCharCoordinates(PLAYER_PED))
			local show = false
            ...
            if show then 
                if sampIsPlayerPaused(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)) == true) then                    
                    rpc.state = sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) .. "[" .. select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)) .. "] | Íàõîäèòñÿ â AFK"
                end
            end
			...
			-- 		local modelscar = getNameOfVehicleModel(getCarModel(vehicle))
			-- 		rpc.state = string.format("Åäåò íà %s â: %s", getGxtText(modelscar), zone)
			-- 	else
			-- 		rpc.state = string.format("Íàõîäèòñÿ: %s", zone)
...
function GetPlayerInZones(x, y, z)
    local streets = {
    ...
    return 'Íåâîçìîæíî óçíàòü, âîçìîæíî â èíòåğüåğå'
end
